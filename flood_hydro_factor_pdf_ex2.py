...
# Historical scans have thick, slightly skewed vertical rules.  Geometry
# detection already crops two pixels inside each rule; clearing another three
# pixels inside the extracted cell prevents the remaining rule edge from being
# enlarged into an OCR character such as ``|`` or ``1``.
LEGACY_CELL_EDGE_CLEAR_PX = 3
...
def recognize_dynamic_legacy_schema(
    ...
    for panel_index, boundaries in enumerate(panel_boundaries):
        for physical_index in optional_physical_indices:
            ...
            if crop.size == 0:
                continue
            crop = clear_legacy_cell_edges(crop)
            variants = {
...
def detect_legacy_multicolumn_row_candidates(
    ...
    for physical_index in range(2, min(5, physical_count)):
        ...
        source = clear_legacy_cell_edges(
            image[data_top:table_bottom, cell_left:cell_right].copy()
        )
        if source.size == 0:
            continue
...
def assign_legacy_cells_from_dual_anchors(
    ...
    for physical_index in range(3, field_count):
        ...
        for row_index, (band_top, band_bottom) in enumerate(water_bands):
            ...
            cell_source = clear_legacy_cell_edges(
    ...
    time_left = round(boundaries[2]) + 2
    time_right = round(boundaries[3]) - 2
    for row_index, (band_top, band_bottom) in enumerate(time_bands):
        cell_source = clear_legacy_cell_edges(
            image[band_top:band_bottom, time_left:time_right].copy()
        )
        gray = cv2.cvtColor(cell_source, cv2.COLOR_BGR2GRAY)
        ink = cv2.threshold(gray, 175, 255, cv2.THRESH_BINARY_INV)[1]
    ...
    for column_index in (1, 0):
        cell_left = round(boundaries[column_index]) + 2
        cell_right = round(boundaries[column_index + 1]) - 2
        column_source = clear_legacy_cell_edges(
            image[data_top:table_bottom, cell_left:cell_right].copy()
        )
        gray = cv2.cvtColor(column_source, cv2.COLOR_BGR2GRAY)
        ...
        for component_index in range(1, count):
            ...
            if len(meaningful_rows) >= 2:
                # A genuinely tall connected component contains substantial
                # ink on both sides of an auxiliary-anchor midpoint.  Split
                # only this component; each piece is owned by exactly one row.
                for owner in meaningful_rows:
                    band_top, band_bottom = time_bands[owner]
                    relative_top = max(0, band_top - data_top)
                    relative_bottom = min(ink.shape[0], band_bottom - data_top)
                    row_masks[owner][relative_top:relative_bottom] = cv2.bitwise_or(
                        row_masks[owner][relative_top:relative_bottom],
...
def clear_legacy_cell_edges(cell: np.ndarray) -> np.ndarray:
    """Whiten residual vertical-rule pixels at both sides of a cell crop."""
    if cell.size == 0:
        return cell
    cleaned = cell.copy()
    margin = min(
        LEGACY_CELL_EDGE_CLEAR_PX,
...
def prepare_legacy_water_part(
    ...
    if cell.size == 0:
        return None
    margin = min(LEGACY_CELL_EDGE_CLEAR_PX, max(1, cell.shape[1] // 8))
    if clear_left:
        cell[:, :margin] = 255
    if clear_right:
        cell[:, -margin:] = 255
...
def choose_legacy_numeric_candidate(
    ...
    plausible_candidates = [item for item in evaluated if item["plausible"]]
    if not evaluated:
        return ("", 0.0), "", "", ""
...
def recognize_legacy_station(
    ...
    for left_ratio, right_ratio, upper_pad in (
        ...
        crop = image[
            max(top + upper_pad, 0):min(data_top - 4, image.shape[0]),
            round(left + width * left_ratio):round(right - width * right_ratio),
        ]
    ...
    station, text, score = max(candidates, key=lambda item: item[2])
    return {
        "station": station,
...
def recognize_legacy_station_markers(
    ...
    body_markers = recognize_station_markers(
        image,
        recognizer,
        table_left,
        table_right,
...
def reconstruct_legacy_page(
    ...
    for panel_index, boundaries in enumerate(panel_boundaries):
        ...
        for row_index, center_y in enumerate(rows):
            ...
            for physical_index, column_index in enumerate(canonical_indices):
                ...
                if column_index == 2:
                    ...
                    if time_mode == "hour_only":
                        ...
                        minute_right = max(
                            minute_left,
                            cell_width - LEGACY_CELL_EDGE_CLEAR_PX,
                        )
                    else:
                        hour_right = min(
                            cell_width, max(2, time_colon_x - 2)
                        )
                        minute_left = min(cell_width, time_colon_x + 2)
                        minute_right = max(
                            minute_left,
                            cell_width - LEGACY_CELL_EDGE_CLEAR_PX,
                        )
            ...
            if date_owned_cells:
                pair_left = round(boundaries[0]) + 2
                pair_right = round(boundaries[2]) - 2
    ...
    for panel_index, rows in enumerate(panel_rows):
        ...
        for row_index, center_y in enumerate(rows):
            ...
            if split_water and not current_water_valid:
            ...
            for field, column_index in (("月", 0), ("日", 1), ("时分", 2)):
                ...
                add_context_candidate(
                    field,
                    normalize_legacy_numeric(source_text, column_index),
                    source_score,
                    "whole-page",
                )
                add_context_candidate(
                    ...
                    training_samples.append(
                        {
                            ...
                            "include": "是" if correct_label else "待确认",
...
def resolve_legacy_calendar_time_context(
    ...
    # Derive the final inherited date before comparing time candidates.
    inherited_dates: list[tuple[int, int] | None] = []
    current_month = None
    current_day: int | None = None
    for row in rows:
        explicit_month = parse_integer(row["values"][0], 1, 12)
        explicit_day = parse_integer(row["values"][1], 1, 31)
        month_changed = False
        if explicit_month is not None:
            if current_month is None or explicit_month == current_month:
                current_month = explicit_month
            else:
                distance = (explicit_month - current_month) % 12
                if 1 <= distance <= 6:
                    current_month = explicit_month
                    current_day = None
                    month_changed = True
        if current_month is not None and explicit_day is not None:
            if explicit_day <= legacy_days_in_month(current_month) and (
                current_day is None
                or month_changed
                or explicit_day >= current_day
            ):
                current_day = explicit_day
        inherited_dates.append(
            (current_month, current_day)
            if current_month is not None and current_day is not None
            else None
        )

    # --- Time: choose only legal 24-hour candidates that preserve row order.
    start = 0
    while start < len(rows):
        date = inherited_dates[start]
        end = start + 1
        while end < len(rows) and inherited_dates[end] == date:
            end += 1
        previous_minutes: int | None = None
        for index in range(start, end):
            row = rows[index]
            if locked(row, "时分"):
                parsed = parse_legacy_clock(row["values"][2])
                if parsed is not None:
                    previous_minutes = parsed[1]
                continue
            current = parse_legacy_clock(row["values"][2])
            options = legacy_context_options(
                row,
                "时分",
                parse_legacy_clock,
            ...
            for following_index in range(index + 1, end):
                parsed_next = parse_legacy_clock(
                    rows[following_index]["values"][2]
                )
                if parsed_next is not None:
                    next_minutes = parsed_next[1]
                    break
            upper_bound = (
                next_minutes
                ...
                and (
                    ...
                    or next_minutes >= previous_minutes
                )
                else None
            )
            current_valid = bool(
                ...
                and (
                    previous_minutes is None
                    or current[1] >= previous_minutes
                )
                and (upper_bound is None or current[1] <= upper_bound)
            )
            if not current_valid:
                viable = {
                    ...
                    if (
                        previous_minutes is None
                        or minutes >= previous_minutes
                    )
                    and (upper_bound is None or minutes <= upper_bound)
                }
                if viable:
                    chosen_minutes, detail = max(
                        viable.items(), key=lambda item: item[1][0]
                    )
                    chosen_value = detail[1]
                    old_value = row["values"][2]
                    row["values"][2] = chosen_value
                    record(
                        row,
                        "时分",
                        old_value,
                        chosen_value,
                        (
                            f"同日上一时刻={previous_minutes}分钟，"
                            f"下一时刻={upper_bound}分钟"
                        ),
                        detail[2],
                    )
                    current = (chosen_value, chosen_minutes)
            if current is not None:
                previous_minutes = current[1]
        start = end
...
def sync_context_choices_to_training_samples(
    ...
    for sample in samples:
        ...
        if not label or sample.get("correct_label") == label:
            continue
